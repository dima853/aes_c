...
/*
- Standard compliance: The array contains 14 constants, which corresponds to the AES standard for AES-256.
- Correct values: The values of the constants (0x01, 0x02, 0x04, ...) are correct for RCON.
- Memory optimization: The code saves memory because it stores only the necessary constants.
- Is used in the G-function when expanding the key.
*/
...
void aesEncrypt(const uint8_t *input, const uint8_t *key, uint8_t *output)
{
    ...
    uint8_t expandedKey[Nb * (Nr + 1) * WORD_SIZE];
    ...
    for (int round = 1; round < Nr; round++)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            for (int j = 0; j < 4; j++)
            {
                roundKey[j][i] = expandedKey[(round * Nb + i) * 4 + j];
            }
        }
        addRoundKey(state, roundKey);
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            roundKey[j][i] = expandedKey[(Nr * Nb + i) * 4 + j];
        }
    }
    addRoundKey(state, roundKey);
